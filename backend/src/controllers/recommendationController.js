...
const recommendFriends = async (req, res) => {
  ...
  try {
      ...
      // Find other users who share mutual friends, but are not already friends with the user
      const recommendations = await User.find({ 
          _id: { $ne: userId, $nin: friendsOfUser },  // Exclude the user and their current friends
